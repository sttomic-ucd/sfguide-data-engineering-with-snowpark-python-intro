...
#        print(f"Skipping non-app folder {directory_path}")
    ...
    # # os.system(f"snow snowpark deploy --replace --temporary-connection --account $SNOWFLAKE_ACCOUNT --user $SNOWFLAKE_USER --role $SNOWFLAKE_ROLE --warehouse $SNOWFLAKE_WAREHOUSE --database HOL_DB")
    os.system(f"snow snowpark deploy --replace --temporary-connection --account $SNOWFLAKE_ACCOUNT --user 'STT0SANDBOX0AWS' --role $SNOWFLAKE_ROLE --warehouse $SNOWFLAKE_WAREHOUSE --database HOL_DB")
    # os.system("ls -l")
